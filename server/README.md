...
| `npm run ai:calibrate` | 测量各内容类型的 token 用量并给出推荐额度（见 6.5） |
...
### AI 额度与模型能力（`008` / `009` / `010`）

| 表 / 列 | 说明 |
|---|---|
| `user_ai_limits` | 每类内容的输出上限、输入上限、推理倍数、`allow_thinking`。**留空 = 跟随系统推荐值** |
| `model_capabilities` | 按「端点 + 模型」记住探测结果：是否推理型、是否支持 `response_format`、`thinking_disable_mode` |
| `model_capabilities.thinking_disable_mode` | 该端点可用的「关闭思考」写法：`thinking` / `reasoning_effort` / `unsupported` |

`model_capabilities` 存在的理由很具体：这些结论原本只在内存里，服务器一重启就丢，
表现是「重启后第一次生成必然失败」——现象随机、极难定位。落库后进程启动即恢复结论。

注意只有**偏离默认值**的探测结果才会写库：默认认定「支持 `response_format`」、
「用 `thinking` 关闭思考」，这两种情况连一次调用都没失败过，写库只是无意义的 IO。
所以该表为空是正常状态，不代表没探测过；一旦端点拒绝了某个参数，降级结论会立刻落库。
...
### 6.5 AI 内容生成与成本控制（PRD 4.3 / 4.3.6 / 4.8）

#### 成本控制的七个层次

每一次生成请求都严格按下面顺序走，越靠前越省钱：

| 顺序 | 手段 | 效果 |
|---|---|---|
| 1 | **根本不用 AI**：错因分析、易混词对比卡片是纯算法 | 成本恒为 0 |
| 2 | **跳过模型的内部思考**：请求里带 `thinking:{type:"disabled"}` | 输出 token 从 1000+（全被思考吃掉）降到约 410，失败率归零 |
| 3 | **缓存池命中**：相同词组合 + 难度 + 目标，72 小时内跨用户复用 | 0 成本，不扣额度 |
| 4 | **没有可用 Key**：返回本地组装的复习清单 | 0 成本，接口仍可用 |
| 5 | **合批**：N 个错词的卡片合并成 1 次请求 | 固定开销只付一次（逐词调用是 N 倍） |
| 6 | **精简 Prompt**：目标词不带例句、释义最多 2 条、画像压成一行 | 输入 token 约降到旧版的三分之一 |
| 7 | **预算兜底**：按输入预算自动减少目标词；调用失败降级为本地内容 | 单次成本有上限，失败不重复烧钱 |

另外还有 **历史去重**：用词级 3-gram Jaccard 近似摘要向量相似度，达到
`DEDUP_SIMILARITY_THRESHOLD`（0.85）就换话题重新生成一次（PRD 4.4 第 4 条）。

以下几个实现细节值得单独说明：

**跳过内部思考是成本与稳定性上收益最大的一处改动。** 推理型模型有相当大概率把整个
`max_tokens` 预算花在 `reasoning_content` 上，正文一个字都不写，而这些 token 照常计费。
同一 Prompt、同一模型各跑 3 次的实测：

| 请求方式 | 思考次数 | 写出正文次数 | 平均输出 token | 耗时 |
|---|---|---|---|---|
| 不传参数 | 3/3 | 0/3 | 1000（全被吃光） | 约 12 秒 |
| `thinking:{type:"disabled"}` | 0/3 | 3/3 | 413 | 约 3 秒 |
| `reasoning_effort:"none"` | 0/3 | 3/3 | 408 | 约 3 秒 |

因此**默认就跳过思考**。并非所有 OpenAI 兼容端点都认这两个参数，不认的会返回 400；
系统会依次降级：`thinking` → `reasoning_effort` → 不带参数，
并把「该端点认哪种写法」记进 `model_capabilities.thinking_disable_mode`，避免每次重启都白试一次。
用户若想要更细致的输出，可以在设置页显式打开「允许模型先内部思考」（存 `user_ai_limits.allow_thinking`）。

**缓存键刻意不含话题。** 话题是每次随机抽的（PRD 4.4 要求「相同词表不同输出」），
如果把它放进缓存键，同一批词第二次请求只有 1/8 的概率命中，等于白花钱。
话题的随机性作用于「未命中才真正生成」的那一次，缓存负责压掉重复请求——
这正是 PRD 4.3.6 第 1 条的要求。用户确实想要新的一篇时用 `forceNew` 显式绕开。

**System Prompt 每次请求都要发送，属于固定成本，所以写得紧凑**，但它包含的六条预设约束
一条都不能少，它们正是「不跑题、不超纲、不被当成通用聊天工具」的保证（PRD 4.4 / 4.8）。

**合批是最关键的一处。** 逐词生成卡片会把 System Prompt 与固定开销乘以 N；
`POST /content/error-cards` 接受 `wordIds[]`（不传则自动取最近错得最多的词），一次最多合并 6 个。

**生成前可以预估。** `POST /content/preflight` 在本地组装 Prompt 并估算 token，**不调用模型**，
返回预计输入 token、输出上限、最坏成本、是否会命中缓存、需要调用几次。
前端用它实现「点之前先告诉用户要花多少」，并且**所有生成都必须由用户显式点击**，不自动触发。

**生成不满意可以加码重来。** 短文与错词卡片的请求体接受 `boost`（1-4，默认 1），
输出上限与预估成本同步放大，前端把「最坏约 ¥0.017」直接写在按钮旁边，由用户自己决定值不值得。
注意 `boost` 只放大「天花板」，模型写完就停，所以它与真实花费并不是线性关系。

#### 用户自带 API Key（PRD 4.8）

| 能力 | 说明 |
|---|---|
| 加密存储 | AES-256-GCM，密文格式 `v1:iv:authTag:ciphertext`；被篡改会解密失败而不是解出垃圾 |
| 永不回传明文 | 接口只返回打码值（`sk-abc****cdef`），用户靠它确认「我填过哪一个」 |
| 保存前先验证 | 保存时先走 `GET /models`（0 token）+ 一次 64 token 的探测调用；失败不入库，返回 400 `AI_KEY_UNVERIFIED` |
| 主密钥 | 优先 `AI_KEY_ENCRYPTION_SECRET`，未配置则从 `JWT_SECRET` 派生；两者变更后用户需重填 |
| 随时删除 | `DELETE /ai/keys/:id`，删除后回落到官方额度或本地内容 |
| 额度区别对待 | 花官方额度的按 `AI_DAILY_*_QUOTA` 硬限制；用户自带 Key 时放宽 `AI_USER_KEY_QUOTA_MULTIPLIER`（默认 20 倍），仅用于防止被当成免费代理 |
| 预设约束不可绕过 | 无论用谁的 Key，生成请求一律经过 `promptTemplates.js`，无法改成通用聊天 |

预设服务商：DeepSeek、OpenAI、Moonshot、智谱 GLM，以及任意兼容 OpenAI 协议的端点（中转平台、本地 Ollama）。
**服务商预设里的模型名需按官方文档核对**，界面上可以改。
模型名匹配忽略大小写与首尾空白（实测踩过 `deepSea**k**-flash` 这类手打错误把可用配置挡在门外）。

#### 成本如何被看见

- `GET /ai/usage`：今日与近 30 天的调用次数、缓存命中、输入/输出 token 与估算成本
- `GET /ai/status`：一次性返回模式、额度、Key 列表、用量与定价说明，设置页只需调这一个接口
- 每次生成响应里带 `usage`（真实用量）与 `preflight`（生成前预估）；
  `usage` 里还有 `retried` / `retryReason` / `firstAttemptBudget` / `retryBudget`——
  自动重试过就把「首次失败的原因、额度从多少加宽到多少」如实展示出来
- 单价通过 `AI_PRICE_*` 配置，**默认值只是参考，换厂商或官方调价后必须同步修改**；
  也可以把单价全设为 0，界面上就只显示 token 数

#### 降级而不是报错

AI 不可用时（未配置 Key、Key 无效、触发限额、超时、上游故障）接口**不返回 5xx**，
而是返回 200 + 本地组装的内容：

```json
{
  "source": "template",
  "degradedReason": "reasoning_exhausted",
  "message": "AI 生成失败：模型把 1000 token 的输出额度全用在了内部思考上，没有写出正文（首次给了 1000 token，自动放宽到 6000 后仍不足、内部思考写了约 11007 字）。已改为本地复习清单。到设置页关掉「允许模型先内部思考」通常就能解决；也可以改用非推理模型（如 deepseek-flash）。",
  "content": { "meta": { "template": true, "items": [] } }
}
```

**降级提示一律带具体数字。** 早期版本只报一句「AI 暂时不可用」，
用户既不知道卡在哪一步，也不知道自己只要关个开关就能解决——那等于没给信息。
现在每一类失败都会说明：用了哪个数字的额度、是否自动重试过、重试加宽到多少、下一步该做什么。

本地内容是**结构化的复习清单**（释义、音标、易混词对比、记忆提示），而不是硬编一篇假短文——
没有大模型就写不出短文，假装生成了只会误导用户。前端据 `source` 字段（`ai` / `cache` / `template`）
切换渲染方式。

只有 `AiCallError`（真正来自上游的失败）才允许降级；其它异常一律抛出。
这条边界很值钱：实测有一个变量名写错导致的 `ReferenceError` 被 catch 吞掉，
表现成「AI 生成总是失败、走本地兜底」，排查方向全被带偏到 Key 和模型上去了。

#### 零 AI 成本的能力（明确固化）

- **错因分析**：`errorAnalysis.js` 纯规则引擎，每次作答都会跑，永远不调用 AI
- **易混词对比卡片**：`wordRelations.js` 的编辑距离 + 释义相似度算法
- **复习清单**：释义、音标、易混词全部取自数据库

这三项构成「一分钱不花也能正常背单词」的底线。

#### 额度怎么定：先量再定

`npm run ai:calibrate` 会测出「生成一次到底需要多少 token」：

```bash
npm run ai:calibrate                              # 用真实词库构建 Prompt，测量输入 + 估算输出（0 成本）
npm run ai:calibrate -- --email=你@example.com    # 用已保存的 Key 真实调用一次，读服务商返回的真实用量
npm run ai:calibrate -- --samples=3               # 每种内容测 3 次取平均
```

真实调用实测（DeepSeek，已关掉内部思考）：

| 内容类型 | 输入 token | 输出 token | 系统默认上限 |
|---|---|---|---|
| 巩固短文（230 词） | 479 | **567** | 1000 |
| 阅读理解题（3 题） | 1578 | **1454** | 2000 |
| 错词卡片（6 张） | 1270 | **2431** | 3200 |
| 薄弱点小结（一周） | 419 | **349** | 600 |

**关键认知：`max_tokens` 是「天花板」而不是「预留」。** 模型写完就停，用不到的额度不收费。
所以调大它几乎不花钱，调小却会真的导致 JSON 被截断、生成失败——这个取舍是不对称的。
默认值取实测值的约 1.3-1.6 倍，既不浪费也不会截断。

> 这里踩过一次坑：最早的默认值是照着一份「想象中精简的输出样本」定的（短文 320、
> 理解题 487、卡片 556），而模型实际写的字是这些的两三倍。结果是**第一次调用必然被截断**，
> 系统再花一次调用去重试——既慢又贵。教训是：额度只能拿真实调用去量，
> `calibrateAi.js` 里的参考样本也已按真实长度改写，避免再次误导。

#### 用户可自定义额度

设置页可以调，存 `user_ai_limits` 表（**留空 = 跟随系统推荐值**，这样以后改默认值时老用户自动受益）：

| 档位 | 短文 | 理解题 | 卡片 | 小结 | 输入上限 |
|---|---|---|---|---|---|
| 省钱 | 800 | 1600 | 2400 | 450 | 1200 |
| **均衡（推荐）** | 1000 | 2000 | 3200 | 600 | 2000 |
| 高质量 | 1800 | 3000 | 4500 | 900 | 3500 |

也可以逐项填数字（有范围校验，越界直接 400 而不是静默截断），或传 `null` 让单个字段恢复推荐值。
套用档位**只改数字**，不会动「允许模型先内部思考」这个开关——否则用户手动打开的开关会被静默关掉。

`AI_REASONING_BUDGET_MULTIPLIER`（默认 3）只在**允许内部思考**时才生效：
默认既然会让模型跳过思考，它的表现就与普通模型一致，再乘一遍倍数只会让预估虚高，
用户会以为一次短文要 3000 token（实际只用约 570）。

万一「跳过思考」没生效（端点不认参数）而额度又真被思考吃光，
自动重试**不按倍数硬乘**：实测 1000 × 3 = 3000 恰好落在失败区间里，
所以重试直接抬到 `REASONING_RETRY_FLOOR`（6000，实测安全水位）再试一次。
...
| POST | `/content/preflight` | **生成前预估**：本地算 token，不调用模型（成本 0）；接受 `boost` 以便预先算出加码后的价格 |
| POST | `/content/articles` | 生成巩固短文（命中缓存不扣额度；无 Key 时降级为本地复习清单）；接受 `boost`(1-4) 加大额度重新生成 |
| POST | `/content/quizzes` | `{articleId, count}` 生成理解题 |
| POST | `/content/error-cards` | `{wordIds?:[], limit?, wordId?, boost?}` **批量**生成错词对比卡片，不传词则自动取最近错得最多的 |
| POST | `/content/weak-summary` | 生成 AI 版薄弱点小结 |
| GET | `/content?type=&page=&size=` | 历史生成记录（不含正文） |
| GET | `/content/:id` | 取单条完整内容 |

> 以上接口在 AI 不可用时都返回 **200 + `source: 'template'`** 的本地内容，而不是 5xx。
> 降级响应里的 `message` 会带上具体数字（额度、思考长度、是否重试过），
> `degradedReason` 是机器可读的原因（如 `reasoning_exhausted`、`truncated`、`invalid_json`）。

### AI 设置与用量 `/ai`

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/ai/status` | 设置页用：模式（用谁的 Key）、额度、Key 列表、用量、定价说明、零成本能力清单 |
| GET | `/ai/usage?days=30` | 调用次数、缓存命中、输入/输出 token、估算成本 |
| GET | `/ai/providers` | 可选服务商预设（Base URL 与默认模型） |
| PUT | `/ai/keys` | 保存自带 Key。**保存前先做连通性测试**，失败返回 400 `AI_KEY_UNVERIFIED` 且不入库 |
| GET | `/ai/keys` | Key 列表，只返回打码值，永不返回明文 |
| DELETE | `/ai/keys/:id` | 删除 Key，删除后回落到官方额度或本地内容 |
| POST | `/ai/keys/test` | 用最小请求（64 token）测试当前 Key |
| POST | `/ai/models` | 拉账号可用模型列表（只调 `GET /models`，**0 token**），供界面做下拉选择 |
| POST | `/ai/keys/verify` | 只校验不保存，可反复调试 |
| GET | `/ai/limits` | 读取当前生效的 token 额度 + 档位 + 实测参考值 + 「跳过思考」的实测证据 |
| PUT | `/ai/limits` | 套用档位 `{preset}`、逐项调整（字段传 `null` 表示恢复推荐值），或 `{allowThinking: true|false}` |
| POST | `/ai/limits/reset` | 全部恢复系统推荐值 |
...
188 个用例 / 32 个套件 → 现为 **378 个用例 / 79 个套件**，约 16 秒跑完，覆盖：
...
- **AI 集成**：用本地假模型替换真实服务，精确断言「发了几次请求、Prompt 里装了什么」——
  Key 加密与篡改检测、保存前校验、Prompt 不含例句、5 个错词只调 1 次、缓存命中不再调用、用量与成本记账、降级不产生费用
- **跳过内部思考**：默认请求必须带 `thinking=disabled`；端点不认时依次降级到 `reasoning_effort`、再不带参数；
  用户显式打开「允许内部思考」时不得带任何相关参数；降级提示必须包含具体数字而不是一句「AI 暂时不可用」
- **重试额度**：小基数乘倍数会落在失败区间（1000 × 3 = 3000）里，因此重试必须抬到 6000 的实测水位
...
| `src/stores/ai.js` | AI 内容与设置：预估、生成、Key、模型下拉、额度、`lastUsage`（含重试原因） |
| `src/views/LoginView.vue` | 登录 / 注册（同一页两个标签） |
| `src/views/OnboardingView.vue` | 五步引导：目标 → 自评词汇量 → 记忆偏好 → 每日量 → 词书确认 |
| `src/views/StudyView.vue` | 背词：选项展示整段释义并加粗首个释义，底部显示本次 token 与成本 |
| `src/views/ArticleView.vue` | AI 巩固内容：预估面板、生成、加码重来（额度 ×2/×3 且标明最坏价格）、用量明细 |
| `src/views/ReviewView.vue` | 今日错词回顾 + 短文回顾（可反复复习，历史短文点开不再花钱） |
| `src/views/SettingsView.vue` | 接入 Key、模型下拉、额度档位、**「允许模型先内部思考」开关（附实测数字）** |
